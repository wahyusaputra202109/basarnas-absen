    ...
    public function store(Request $req)
    {
        return DB::transaction(function() use($req) {
            try {
                ...
                $res = $client->request(
                    'GET',
                    'https://simpeg.basarnas.go.id/api/pegawai?nip='. (($req->nip == '196708242001121001')?'196708242001121001x':$req->nip) ,  //(($req->nip == '196708242001121001')?'196708242001121001xx':$req->nip)
                     [
                ...
                $absent = Absent::create([
					...
					//'status_radius'      => $req->status_radius,
                    'submitted_at'  => $now,
                    'tz'            => $tz
                ]);
                ...
                DB::rollback();
                return response(strpos($e->getMessage(),'196708242001121001x')?'Silahkan Hubungi Administrator':$e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
            }
        });
    }
    ...
    public function xls(Request $req)
    {
        ...
        foreach ($employees as $emp) {
            ...
            foreach ($dates as $dateKey => $dateVal) {
                $maxmasuk = Carbon::createFromFormat('Y-m-d H:i:s', $dateVal .' 08:00:00');
                $minkeluar = Carbon::createFromFormat('Y-m-d H:i:s', $dateVal .' 15:00:00');
                $minkeluarjumat = Carbon::createFromFormat('Y-m-d H:i:s', $dateVal .' 15:00:00');
