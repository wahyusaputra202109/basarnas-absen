<template>
    <el-main style="background-color:rgb(140, 197, 255);padding: 50px 50px">
        <el-row class="h100pc">
                        ...
                        <el-card shadow="always" class="bg-info">
                            <el-row>
                                ...
                                <el-col :xs="24" :md="20" :lg="20">
                                    <h2>SIAGA - Absensi Online Basarnas</h2>
                                    <hr/>
                                    <h5></h5>
                                </el-col>
                            </el-row>
                        ...
                        <el-card shadow="always" v-if="isSubmitted">
                            <el-row>
                                ...
                                <el-col :xs="24" :md="19" :lg="19">
                                    ...
                                    <el-row class="mb10">
                                        <el-col :xs="24" :sm="7" :md="6" :lg="5" :span="5">
                                            <span><b>Siaga Jabatan</b></span>
                                        </el-col>
                                        <el-col :xs="1" :sm="1" :md="1" :lg="1" :span="1">:</el-col>
                                        <el-col :xs="20" :sm="16" :md="17" :lg="18" :span="18">
                                            <span>{{ dataEmp.siaga_jabatan }}</span>
                                        </el-col>
                                    </el-row>
                                    <el-row class="mb10">
                                        <el-col :xs="24" :sm="7" :md="6" :lg="5" :span="5">
                                    ...
                                    <el-row class="mb5">
                                        <el-col :span="24" class="text-center">
                                            <span class="fs24 text-danger"><b>Absensi {{ dataEmp.shift }} Berhasil</b></span>
                                        </el-col>
                                    </el-row>
                                    <el-row>
                        ...
                        <el-card shadow="always" v-else>
                            <el-row>
                                <el-col :xs="24" :md="15" :lg="15">
                                    ...
                                    <!-- <p class="mtn">Penetapan Jam Pada Bulan Ramadhan 1441H</p> -->
                                    <el-table :data="arrShift" border>
...
export default {
    data() {
        return {
            ...
            tableDataInfo: [{
                ...
                sore: '16.01 s/d 19.00'
            },{
                ket: 'Shift 2',
                pagi: '19.01 s/d 20.00',
                sore: '04.00 s/d 07.00'
            }],
            dataEmp: [],
            isLoading: false,
            isSubmitted: false,
    ...
    mounted() {
        var allowedTZ = [ 'WIB', 'WITA', 'WIT' ];
        var tz = this.$route.params.tz ? this.$route.params.tz.toUpperCase() : 'WIB';
        tz = allowedTZ.indexOf(tz) >=0 ? tz : 'WIB';

        this.getDateTime(tz);
        setInterval(() => { this.getDateTime(tz) }, 1000);
    ...
    methods: {
        async getDateTime(tz) {
            this.now = this.now == '' ? new Date() : this.now;

            let vm = this;
            if(this.datetime == '')
                this.$store.dispatch('doGet', { url: '/api/absen/time/'+ tz })
                    .then(res => {
                        ...
                        vm.msgOpen = 'officehour';//res.data.msgOpen;
                        ...
                        this.datetime = this.days[this.now.getDay()] +', '+ this.now.getDate() +' '+ this.months[this.now.getMonth()] +' '+ this.now.getFullYear() +' '+ (this.now.getHours()<10?'0':'') + this.now.getHours() +':'+ (this.now.getMinutes()<10?'0':'') + this.now.getMinutes() +':'+ (this.now.getSeconds()<10?'0':'') + this.now.getSeconds() + ' ' + tz;
                    })
                    .catch(err => {
                        console.log(err);
                    });
        ...
        async doAbsen(model) {
            ...
            let vm = this;
            this.$store.dispatch('doPost', { url: '/api/absen/siaga/store', data: model })
                .then(res => {
                    vm.$notify.success({
                        title: 'Success',
                    ...
                    vm.dataEmp.waktu = this.days[this.now.getDay()] +', '+ this.now.getDate() +' '+ this.months[this.now.getMonth()] +' '+ this.now.getFullYear() +' '+ (this.now.getHours()<10?'0':'') + this.now.getHours() +':'+ (this.now.getMinutes()<10?'0':'') + this.now.getMinutes() +':'+ (this.now.getSeconds()<10?'0':'') + this.now.getSeconds() +' '+ vm.dataEmp.tz;
                    ...
                    if(vm.dataEmp.absenmasukakhir) {
                        ...
                        vm.now = new Date(arrWaktu[0],arrWaktu[1] -1,arrWaktu[2],arrWaktu[3],arrWaktu[4],arrWaktu[5]);
                        vm.dataEmp.absenmasukakhir = this.days[this.now.getDay()] +', '+ this.now.getDate() +' '+ this.months[this.now.getMonth()] +' '+ this.now.getFullYear() +' '+ (this.now.getHours()<10?'0':'') + this.now.getHours() +':'+ (this.now.getMinutes()<10?'0':'') + this.now.getMinutes() +':'+ (this.now.getSeconds()<10?'0':'') + this.now.getSeconds() +' '+ vm.dataEmp.tz;
                    }
                    if(vm.dataEmp.absenkeluarakhir) {
                        // vm.now = new Date(vm.dataEmp.absenakhir);
                        arrWaktu = vm.dataEmp.absenkeluarakhir.split(',').map(d => { return parseInt(d) });
                        vm.now = new Date(arrWaktu[0],arrWaktu[1] -1,arrWaktu[2],arrWaktu[3],arrWaktu[4],arrWaktu[5]);
                        vm.dataEmp.absenkeluarakhir = this.days[this.now.getDay()] +', '+ this.now.getDate() +' '+ this.months[this.now.getMonth()] +' '+ this.now.getFullYear() +' '+ (this.now.getHours()<10?'0':'') + this.now.getHours() +':'+ (this.now.getMinutes()<10?'0':'') + this.now.getMinutes() +':'+ (this.now.getSeconds()<10?'0':'') + this.now.getSeconds() +' '+ vm.dataEmp.tz;
                    }
                })
                .catch(err => {
            ...
            this.$store.dispatch('doGet', { url: '/api/siaga/shift/dt', data: { page: 1, search: '', viewData: 100, sort: [] } })
                .then(res => {
                    ...
                    for(var item in data) {
                        vm.arrShift.push({
                            ket: data[item].nama,
                            pagi: data[item].mulaimasuk +' s/d '+ data[item].selesaimasuk,
                            sore: data[item].mulaipulang +' s/d '+ data[item].selesaipulang
                        });
                    }
                })
        },
