            ...
            <el-row>
                <el-col :span="24" class="pl10 pr10">
                    <el-card body-style="padding: 0;background-color: #f5f5f5">
                        <data-table :get-data="getData" ref="table" :columns="columns" :display="'25'" :create-visible="hasPermission('create position')" :create-route="{ name: 'siaga-position-create' }">
                            <span slot="name" slot-scope="{ row }">
                                {{ row.name }}
                            </span>
                            <div slot="__" slot-scope="{ row }">
                                <el-button-group>
                                    <el-tooltip class="item" effect="dark" content="Edit item" placement="top-start" v-if="hasPermission('update position')">
                                        <el-button size="mini" icon="el-icon-edit" type="success" @click="doAction('edit', row)"></el-button>
                                    </el-tooltip>
                                    <el-tooltip class="item" effect="dark" content="Delete item" placement="top-start" v-if="hasPermission('delete position')">
...
export default {
    ...
    methods: {
        getData({ page, search, sortParams, viewData }) {
            return this.$store.dispatch('doGet', { url: '/api/siaga/position/dt', data: { page, search, viewData, sort: sortParams } })
                .then(res => {
                    return {
                        data: res.data,
        ...
        doAction(action, row) {
            this.$confirm('Are you sure want to '+ action +' this item?', 'Warning', {
                ...
                .then(_ => {
                    if(action == 'edit')
                        this.$router.push({ name: 'siaga-position-create', params: { id: row.id } });
                    else
                        this.$store.dispatch('doGet', { url: '/api/siaga/position/delete/'+ row.id })
                            .then(res => {
                                this.$refs.table.getTableData();
                                this.$notify.success({
                                    title: 'Success',
                                    message: 'Siaga Jabatan deleted successfully.'
                                });
                            })
                            .catch(err => {
                                this.$notify.error({
