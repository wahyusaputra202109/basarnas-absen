            ...
            <el-row>
                <el-col :span="24" class="pl10 pr10">
                    <el-card body-style="padding: 0;background-color: #f5f5f5">
                        <data-table :get-data="getData" ref="table" :columns="columns" :display="'25'" :create-visible="hasPermission('create role')" :create-route="{ name: 'role-create' }">
                            ...
                            <div slot="__" slot-scope="{ row }">
                                <el-button-group>
                                    <el-tooltip class="item" effect="dark" content="Edit item" placement="top-start">
                                        <el-button size="mini" icon="el-icon-edit" type="success" @click="doAction('edit', row)" v-if="hasPermission('update role')"></el-button>
                                    </el-tooltip>
                                    <el-tooltip class="item" effect="dark" content="Delete item" placement="top-start">
                                        <el-button size="mini" icon="el-icon-delete" type="danger" @click="doAction('delete', row)" v-if="hasPermission('delete role')"></el-button>
                                    </el-tooltip>
                                </el-button-group>
                            </div>
                        </data-table>
...
export default {
    ...
    methods: {
        getData({ page, search, sortParams, viewData }) {
            return this.$store.dispatch('doGet', { url: '/api/roles/dt', data: { page, search, viewData, sort: sortParams } })
                .then(res => {
                    return {
                        data: res.data,
        ...
        doAction(action, row) {
            this.$confirm('Are you sure want to '+ action +' this item?', 'Warning', {
                ...
                .then(_ => {
                    if(action == 'edit')
                        this.$router.push({ name: 'role-create', params: { id: row.id } });
                    else 
                        this.$store.dispatch('doGet', { url: '/api/roles/delete/'+ row.id })
                            .then(res => {
